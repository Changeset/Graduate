...
import ids.storage.Scaffold;
import ids.storage.ScaffoldFactory;
...
import static ids.core.Detect.CONFIG_PATH;
import static ids.core.Detect.FILE_SEPARATOR;
import static ids.core.Detect.IDS_ROOT;

/**
 * @ Author: Xuelong Liao
 * @ Description:
 * @ Date: created in 10:41 2019/1/18
 * @ ModifiedBy:
 */
public abstract class AbstractStorage {
    ...
    private static String configFile = CONFIG_PATH + FILE_SEPARATOR + "ids.core.AbstractStorage.config";

    /**
     * Variables and functions for managing storage
     */
    ...
    static {
        try {
            ...
            SCAFFOLD_PATH = IDS_ROOT + databaseConfigs.getProperty("scaffold_path");
            ...
            SCAFFOLD_PATH = IDS_ROOT + "db/scaffold";
            SCAFFOLD_DATABASE_NAME = "Redis";
            Logger.getLogger(AbstractStorage.class.getName()).log(Level.WARNING,
                    "Loading scaffold configurations from file '" + configFile + "' " +
                            " unsuccessful! Falling back to default settings", e);
        }
    }
    
    protected boolean insertScaffoldEntry(Edge incomingEdge) { return scaffold.insertEntry(incomingEdge); }
     ...
     */
    @Deprecated
    public abstract Edge getEdge(String childVertexHash, String parentVertexHash);
     ...
     */
    @Deprecated
    protected Graph convertStackToGraph(Stack<Vertex> stack)
    {
        Graph graph = new Graph();
        Iterator<Vertex> iter = stack.iterator();
        Vertex previous = iter.next();
